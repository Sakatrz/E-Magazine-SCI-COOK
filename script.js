...
// ── Touch Gestures: Pinch-to-Zoom (Cubit Layar) & Double Tap ──
let touchStartDist = 0;
let touchStartZoom = 1.0;
let lastTapTime = 0;

magazineMain.addEventListener(
  "touchstart",
  (e) => {
    if (e.touches.length === 2) {
      // 2 jari menyentuh layar -> mulai gesture cubit (pinch zoom)
...
magazineMain.addEventListener(
  ...
  (e) => {
    if (e.touches.length === 2 && touchStartDist > 0) {
      if (e.cancelable) e.preventDefault(); // cegah scroll default saat sedang pinch zoom
      const dx = e.touches[0].clientX - e.touches[1].clientX;
      const dy = e.touches[0].clientY - e.touches[1].clientY;
      const currentDist = Math.hypot(dx, dy);
      const scale = currentDist / touchStartDist;

      let newZoom = +(touchStartZoom * scale).toFixed(2);
...
magazineMain.addEventListener(
  ...
  (e) => {
    ...
    // Double tap gesture (1 jari, ketuk 2x cepat untuk zoom in / reset)
    if (e.changedTouches.length === 1 && !e.target.closest(".trigger")) {
      const now = Date.now();
      if (now - lastTapTime < 300 && now - lastTapTime > 0) {
        if (currentZoom > 1.0) {
          handleZoom(0); // reset ke 100%
        } else {
          currentZoom = 1.6; // zoom in ke 160%
          fitContainer();
          updateHeaderUI();
        }
      }
